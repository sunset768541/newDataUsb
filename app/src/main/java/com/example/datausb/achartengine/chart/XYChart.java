 ...
 */
package com.example.datausb.achartengine.chart;
...
import com.example.datausb.DataWR;
import com.example.datausb.achartengine.model.Point;
import com.example.datausb.achartengine.model.SeriesSelection;
...
import com.example.datausb.achartengine.renderer.BasicStroke;
import com.example.datausb.achartengine.renderer.DefaultRenderer;
import com.example.datausb.achartengine.renderer.SimpleSeriesRenderer;
import com.example.datausb.achartengine.renderer.XYMultipleSeriesRenderer;
import com.example.datausb.achartengine.renderer.XYMultipleSeriesRenderer.Orientation;
import com.example.datausb.achartengine.renderer.XYSeriesRenderer;
import com.example.datausb.achartengine.util.MathHelper;
...
public abstract class XYChart extends AbstractChart {
  ...
  public float []point=new float[16384];
  /**
  ...
  public void setmDataset(XYMultipleSeriesDataset data){
    mDataset=data;
  }
  public void setmDataset(float[] data){
    point=data;
  }
  /**
  ...
  //  Log.e("XYchart","draw函数");
    ...
    drawBackground(mRenderer, canvas, x, y, width, height, paint, false, DefaultRenderer.BACKGROUND_COLOR);
    ...
    int maxScaleNumber = -Integer.MAX_VALUE;//-2147483647
    for (int i = 0; i < sLength; i++) {//sLength=4
    ...
    for (int i = 0; i < maxScaleNumber; i++) {
      ...
      Log.e("i="+Integer.valueOf(i).toString()," minX="+Double.valueOf(minX[i]).toString()+" maxX="+Double.valueOf(maxX[i]).toString()+" minY="+Double.valueOf(minY[i]).toString()+" maxY"+Double.valueOf(maxY[i]).toString());
    }
    double[] xPixelsPerUnit = new double[maxScaleNumber];//2147483647长度
    double[] yPixelsPerUnit = new double[maxScaleNumber];
    for (int i = 0; i < sLength; i++) {//4ci
      XYSeries series = mDataset.getSeriesAt(i);//取出第一个XYseries
      int scale = series.getScaleNumber();//scale默认0
      if (series.getItemCount() == 0) {//返回XYmap的大小
        continue;
      }
      if (!isMinXSet[scale]) {
    ...
    for (int i = 0; i < maxScaleNumber; i++) {//int最大值
      if (maxX[i] - minX[i] != 0) {
        xPixelsPerUnit[i] = (right - left) / (maxX[i] - minX[i]);//maxX为横坐标的最大值，minX为横坐标最小值
      }
      if (maxY[i] - minY[i] != 0) {
    ...
    //  Log.e("i="+Integer.valueOf(i).toString(),"  xPixelsPerUnit【i】="+Double.valueOf(xPixelsPerUnit[i]).toString());
    }
 ...
 */
      ...
      synchronized (series) {
        ...
        int startIndex = -1;
        int mm=0;
        for (Entry<Double, Double> value : range.entrySet()) {//执行8192
          double xValue = value.getKey();
          double yValue = value.getValue();
//          Log.e("xValue=", Double.valueOf(xValue).toString());//0-8192
//          Log.e("yValue=", Double.valueOf(yValue).toString());//--32767
         ...
         // Log.e("point前",Float.valueOf(points.get(12)).toString());//
          ...
          //  Log.e("计算X"," left="+Integer.valueOf(left).toString()+" scale="+Float.valueOf(scale).toString()+" xPixelsPerUnit[scale]="+Double.valueOf(xPixelsPerUnit[scale]).toString()+" xValue="+Double.valueOf(xValue).toString()+" minX[scale]="+Double.valueOf(minX[scale]).toString());//只执行了2
        ...
        /**
         * 获得了适配屏幕的point长度为16384
         */
      //  Log.e("point后",Integer.valueOf(points.size()).toString()+" for执行次数="+Integer.valueOf(mm).toString());//输入已经处理过
        ...
        if (points.size() > 0) {
//          Log.e("point",Float.valueOf(points.get(12)).toString());//输入已经处理过
  ...
  protected void drawSeries(XYSeries series, Canvas canvas, Paint paint, float[] pointsList,
                            XYSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, Orientation or,
                            int startIndex) {
    Log.e("XYCAHRT","drawSeries");
  ...
  protected void drawPoints(Canvas canvas, Paint paint, float[] pointsList,
                            XYSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, int startIndex) {
    if (isRenderPoints(seriesRenderer)) {
      ScatterChart pointsChart = getPointsChart();
      if (pointsChart != null) {
  ...
  protected void drawChartValuesText(Canvas canvas, XYSeries series, XYSeriesRenderer renderer,
    ...
    if (points.length > 2) { // there are more than one point
      // record the first point's position
      ...
      for (int k = 0; k < points.length; k += 2) {
        if (k == 2) { // decide whether to display first two points' values or
          ...
          if (Math.abs(points[2] - points[0]) > renderer.getDisplayChartValuesDistance()
            ...
            // first point
            drawText(canvas, getLabel(renderer.getChartValuesFormat(), series.getY(startIndex)),
                    points[0], points[1] - renderer.getChartValuesSpacing(), paint, 0);
            // second point
            drawText(canvas,
                    getLabel(renderer.getChartValuesFormat(), series.getY(startIndex + 1)),
