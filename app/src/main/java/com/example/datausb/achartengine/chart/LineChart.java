...
import com.example.datausb.achartengine.renderer.XYSeriesRenderer.FillOutsideLine;
...
public class LineChart extends XYChart {
   ...
   */
  public void drawSeries(Canvas canvas, Paint paint, float[] points, XYSeriesRenderer renderer,
                         float yAxisValue, int seriesIndex, int startIndex) {
    //Log.e("LineChart","drawSerier函数");
    ...
    for (FillOutsideLine fill : fillOutsideLine) {
      if (fill.getType() != FillOutsideLine.Type.NONE) {
        ...
        float[] fillPoints = new float[points.length];
        int[] range = fill.getFillRange();
        if (range == null) {
          fillPoints=points;
        } else {
          if (points.length > range[0] * 2 && points.length > range[1] * 2) {
        ...
        if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE
          ...
          fillPoints=new float[fillPoints.length];
          boundsPoints.toArray(new Float[]{});//
